...
async function seedBlok(blokConfig, urutanBlok) {
    ...
    console.log(`Blok ${blokConfig.kode} (${blokConfig.judul}): ${blokConfig.pertanyaan.length} pertanyaan di-seed (mode aman, jawaban lama tidak dihapus).`);
}

async function main() {
    console.log("Memulai seed instrumen (mode aman) — data jawaban wawancara yang sudah ada TIDAK akan dihapus...\n");

    for (let i = 0; i < SEMUA_BLOK.length; i++) {
        await seedBlok(SEMUA_BLOK[i], i);
    }

    console.log("\nSeed instrumen selesai. Pertanyaan/opsi lama yang sudah tidak ada di bank_kuesioner tetap tersimpan di database (tidak otomatis dihapus) supaya histori jawaban warga yang mengacu ke situ tetap aman.");
}
